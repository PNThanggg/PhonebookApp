...
import com.app.phonebook.base.extension.getColorForState
import com.app.phonebook.base.extension.throwIfMissingAttrs
import com.app.phonebook.base.utils.TextColorUtil
import com.app.phonebook.helpers.onUpdate
...
class FastScrollerView @JvmOverloads constructor(
    ...
    defStyleRes: Int = R.style.FastScroller
) : LinearLayout(
    ...
    init {
        context.theme.obtainStyledAttributes(
            attrs, R.styleable.FastScrollerView, defStyleAttr, defStyleRes
        ).use { attrsArray ->
            throwIfMissingAttrs(styleRes = R.style.FastScroller) {
                iconSize =
     ...
     *                      See [com.app.phonebook.presentation.view.FastScrollerView.showIndicator].
     * @param useDefaultScroller whether or not this com.app.phonebook.presentation.view.FastScrollerView should automatically scroll
     *                           [recyclerView] when an indicator is pressed.
     *                           See [com.app.phonebook.presentation.view.FastScrollerView.useDefaultScroller].
     */
    @JvmOverloads
    ...
    private fun bindItemIndicatorViews() {
        ...
        itemIndicators.run {
            ...
            while (index <= lastIndex) {
                @Suppress("UNCHECKED_CAST") val textIndicatorsBatch = subList(
                    index,
                    size
                ).takeWhile { it is FastScrollItemIndicator.Text } as List<FastScrollItemIndicator.Text>
    ...
    private fun selectItemIndicator(
        indicator: FastScrollItemIndicator, indicatorCenterY: Int, touchedView: View, textLine: Int?
    ) {
            ...
            if (view.containsY(touchY)) {
                when (view) {
                    ...
                    is TextView -> {
                        ...
                        selectItemIndicator(
                            touchedIndicator, centerY, view, textLine = touchedIndicatorIndex
                        )
                        consumed = true
                    }
                }
    ...
    interface ItemIndicatorSelectedCallback {
        fun onItemIndicatorSelected(
            indicator: FastScrollItemIndicator, indicatorCenterY: Int, itemPosition: Int
        )
    }
}
