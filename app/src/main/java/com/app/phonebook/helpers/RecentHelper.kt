...
//        }
        ...
        cursor?.use {
            ...
            do {
                ...
                if (photoUri.isEmpty() && number.isNotEmpty()) {
                ...
                if (contactIdWithMultipleNumbers != null) {
                    ...
                    if (specificPhoneNumber != null) {
                        ...
                        specificType = context.getPhoneNumberTypeText(
...
//            recentCalls.filter { !context.isNumberBlocked(it.phoneNumber, blockedNumbers) }
    ...
    fun removeAllRecentCalls(activity: BaseActivity<*>, callback: () -> Unit) {
    ...
    fun restoreRecentCalls(
        activity: BaseActivity<*>, objects: List<RecentCall>, callback: () -> Unit
    ) {
        activity.handlePermission(PERMISSION_WRITE_CALL_LOG) {
            if (it) {
                ensureBackgroundThread {
                    val values = objects.sortedBy { call ->
                        call.startTS
                    }.map {
                        ContentValues().apply {
