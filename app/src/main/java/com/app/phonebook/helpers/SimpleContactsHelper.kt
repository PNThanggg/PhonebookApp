...
class SimpleContactsHelper(val context: Context) {
    private fun getAvailableContacts(
        ...
        ensureBackgroundThread {
            ...
            allContacts.forEach {
                ...
                val contact = names.firstOrNull { simpleContact ->
                    simpleContact.rawId == contactId
                }
                ...
                if (contacts.size > 1) {
                    ...
                    if (contacts.any { contact ->
                        ...
                        contacts.subList(1, contacts.size).forEach { contact ->
                            if (contact.phoneNumbers.all { phoneNumber ->
                                    multipleNumbersContact.doesContainPhoneNumber(
                                        phoneNumber.normalizedNumber, context.telephonyManager
                                    )
                                }) {
                                val contactToRemove =
                                    allContacts.firstOrNull { simpleContact -> simpleContact.rawId == contact.rawId }
            ...
            if (isPerson) {
                ...
                if (firstName.isNotEmpty() || middleName.isNotEmpty() || familyName.isNotEmpty()) {
                    val names = if (startNameWithSurname) {
                        arrayOf(
                            prefix, familyName, middleName, firstName, suffix
                    ...
                    val contact = SimpleContact(
                        rawId, contactId, fullName, photoUri, ArrayList(), ArrayList(), ArrayList()
                    )
    ...
    private fun getContactEvents(getBirthdays: Boolean): SparseArray<ArrayList<String>> {
        ...
        val uri = Data.CONTENT_URI
        val projection = arrayOf(
            Data.RAW_CONTACT_ID, Event.START_DATE
        )
    ...
    fun getPhotoUriFromPhoneNumber(number: String): String {
        ...
        try {
            ...
            Log.e(APP_NAME, "getPhotoUriFromPhoneNumber: ${ignored.message}")
        }
    ...
    fun loadContactImage(
        ...
        val placeholder = placeholderImage ?: BitmapDrawable(
            context.resources, getContactLetterIcon(placeholderName)
        )

        val options =
            RequestOptions().diskCacheStrategy(DiskCacheStrategy.RESOURCE).error(placeholder)
                .centerCrop()

        Glide.with(context).load(path).transition(DrawableTransitionOptions.withCrossFade())
            .placeholder(placeholder).apply(options).apply(RequestOptions.circleCropTransform())
            .into(imageView)
    }
        ...
        ensureBackgroundThread {
            ...
            try {
                val options = RequestOptions().format(DecodeFormat.PREFER_ARGB_8888)
                    .diskCacheStrategy(DiskCacheStrategy.NONE).error(placeholder).centerCrop()

                val size = context.resources.getDimension(R.dimen.shortcut_size).toInt()
                val bitmap = Glide.with(context).asBitmap().load(path).placeholder(placeholder)
                    .apply(options).apply(RequestOptions.circleCropTransform()).submit(size, size)
                    .get()
    ...
    fun exists(number: String, privateCursor: Cursor?, callback: (Boolean) -> Unit) {
        SimpleContactsHelper(context).getAvailableContacts(false) { contacts ->
            ...
            if (contact != null) {
                ...
                val privateContacts = MyContactsContentProvider.getSimpleContacts(privateCursor)
