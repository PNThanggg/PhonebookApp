...
import com.app.phonebook.base.extension.getAllContactSources
import com.app.phonebook.base.extension.getByteArray
import com.app.phonebook.base.extension.getIntValue
import com.app.phonebook.base.extension.getLongValue
import com.app.phonebook.base.extension.getPhotoThumbnailSize
import com.app.phonebook.base.extension.getPrivateContactSource
import com.app.phonebook.base.extension.getStringValue
import com.app.phonebook.base.extension.getStringValueOrNull
...
import com.app.phonebook.base.utils.DEFAULT_ORGANIZATION_TYPE
import com.app.phonebook.base.utils.DEFAULT_WEBSITE_TYPE
...
class ContactsHelper(val context: Context) {
        ...
        ensureBackgroundThread {
            ...
            if (context.baseConfig.mergeDuplicateContacts && ignoredContactSources.isEmpty() && !getAll) {
                tempContacts.filter { displayContactSources.contains(it.source) }.groupBy {
                    it.getNameToDisplay().lowercase(Locale.ROOT)
                }.values.forEach { it ->
                    if (it.size == 1) {
    ...
    private fun getDeviceContacts(
        ...
        arrayOf(
            ...
            context.queryCursor(
                uri, projection, selection, selectionArgs, sortOrder, true
            ) { cursor ->
        ...
        for (i in 0 until listIM.size()) {
            val key = listIM.keyAt(i)
            contacts[key]?.listIM = listIM.valueAt(i)
        }
    ...
    private fun getPhoneNumbers(contactId: Int? = null): SparseArray<ArrayList<PhoneNumber>> {
        ...
        context.queryCursor(
            uri, projection, selection, selectionArgs, showErrors = true
        ) { cursor ->
            val id = cursor.getIntValue(Data.RAW_CONTACT_ID)
    ...
    private fun getNicknames(contactId: Int? = null): SparseArray<String> {
        ...
        val uri = Data.CONTENT_URI
        val projection = arrayOf(
            Data.RAW_CONTACT_ID, CommonDataKinds.Nickname.NAME
        )
    ...
    private fun getIMs(contactId: Int? = null): SparseArray<ArrayList<IM>> {
        val listIM = SparseArray<ArrayList<IM>>()
        val uri = Data.CONTENT_URI
        val projection = arrayOf(
            Data.RAW_CONTACT_ID,
        ...
        context.queryCursor(
            ...
            val id = cursor.getIntValue(Data.RAW_CONTACT_ID)
            val ims = cursor.getStringValue(CommonDataKinds.Im.DATA) ?: return@queryCursor
            ...
            if (listIM[id] == null) {
                listIM.put(id, ArrayList())
            }

            listIM[id]!!.add(IM(ims, type, label))
        }
    ...
    private fun getEvents(contactId: Int? = null): SparseArray<ArrayList<Event>> {
        ...
        val uri = Data.CONTENT_URI
        val projection = arrayOf(
            Data.RAW_CONTACT_ID, CommonDataKinds.Event.START_DATE, CommonDataKinds.Event.TYPE
        )
    ...
    private fun getNotes(contactId: Int? = null): SparseArray<String> {
        ...
        val uri = Data.CONTENT_URI
        val projection = arrayOf(
            Data.RAW_CONTACT_ID, CommonDataKinds.Note.NOTE
        )
    ...
    private fun getWebsites(contactId: Int? = null): SparseArray<ArrayList<String>> {
        ...
        val uri = Data.CONTENT_URI
        val projection = arrayOf(
            Data.RAW_CONTACT_ID, CommonDataKinds.Website.URL
        )
    ...
    private fun getContactGroups(
        storedGroups: ArrayList<Group>, contactId: Int? = null
    ...
    private fun getSourcesSelection(
        addMimeType: Boolean = false, addContactId: Boolean = false, useRawContactId: Boolean = true
    ): String {
    ...
    private fun getSourcesSelectionArgs(
        mimetype: String? = null, contactId: Int? = null
    ): Array<String> {
    ...
    private fun getDeviceStoredGroups(): ArrayList<Group> {
        ...
        val projection = arrayOf(
            Groups._ID, Groups.TITLE, Groups.SYSTEM_ID
        )
    ...
    fun getSaveableContactSources(callback: (ArrayList<ContactSource>) -> Unit) {
        ensureBackgroundThread {
            val ignoredTypes = arrayListOf(
                SIGNAL_PACKAGE, TELEGRAM_PACKAGE, WHATSAPP_PACKAGE, THREEMA_PACKAGE
            )
    ...
    fun getDeviceContactSources(): LinkedHashSet<ContactSource> {
        ...
        val contentResolverAccounts = allAccounts.filter {
            if (it.name.isEmpty() && it.type.isEmpty() && allAccounts.none { contactSource ->
                    contactSource.name.lowercase(
                        Locale.getDefault()
                    ) == "phone"
                }) {
    ...
    fun updateContact(contact: Contact, photoUpdateStatus: Int): Boolean {
        ...
        try {
            ...
            ContentProviderOperation.newDelete(Data.CONTENT_URI).apply {
                val selection = "${Data.RAW_CONTACT_ID} = ? AND ${Data.MIMETYPE} = ? "
                val selectionArgs = arrayOf(
                    contact.id.toString(), CommonDataKinds.StructuredPostal.CONTENT_ITEM_TYPE
                )
                withSelection(selection, selectionArgs)
                operations.add(build())
            }
    ...
    private fun addPhoto(
        contact: Contact, operations: ArrayList<ContentProviderOperation>
    ...
    fun getDuplicatesOfContact(
        contact: Contact, addOriginal: Boolean, callback: (ArrayList<Contact>) -> Unit
    ) {
        ensureBackgroundThread {
    ...
    fun getContactsToExport(
        selectedContactSources: Set<String>, callback: (List<Contact>) -> Unit
    ) {
